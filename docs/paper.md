...
*Floors per instance*: the **decision floor** = d disjunct choices + n output digits; the **Kahn floor** = the sum of ready-set sizes along σ's ready-set trajectory (ready = no unplaced predecessor under the closure of hard ∪ decided edges, pruned by failed-element probing, single element, closure only); both reported at c ∈ {2, 2.5, 3} tokens per element. On the four cells the Kahn floor is 12–13 elements (≈ 30 tokens at c = 2.5) and the decision floor 13–16, so the minimal trace is very short and criterion 8 is not binding; converged lengths are reported in absolute tokens next to both floors so that a percentage tax can be read against them. Headroom and the manipulation gate use the Kahn floor; the kill criterion uses the decision floor.
...
4. Whitelist-unigram filler padding never within 10 pp of native at budgets ≥ 0.5 M.
...
GRPO, Dr. GRPO loss, no reward-std scaling, β = 0; 2 prompts × G = 16; LoRA r = 32, α = 64, dropout 0, lr 1e-5 after 10 warm-up steps; training T = 1.0 no top-k/p, evaluation T = 0.6, top-p 0.95, top-k 20; cap 5,120 (6,144 if native forced-close > 20%). Policy log-probs computed under the arm's own mask and renormalized; sampler and trainer asserted to agree within 1e-3. Stopping: two consecutive 50-step intervals with ΔL\_mean < 5% and Δacc < 4 pp on the stopping-eval split, or 400 steps. Manipulation gate: A seed 1 must reach ≥ 30% token reduction from the frozen model at ≤ 5 pp accuracy loss and remain ≥ 2 × the Kahn floor; else λ = 0.3, then λ = 1.0, then G = 32 on A in that order; the passing setting applies to every arm and the pre-tune curve is reported. Run order: A seed 1 (20-step cost measurement, then to convergence) → B seed 1 → warm decision at B1 step 200 → remaining seeds → C\_rand → diagnostics.
...
TBD — table per §4.2 for every cell tried; prompt-format check; instrument positive control ratio; M; oracle floor at c = 2, 2.5, 3; distinct canonical forms in train/eval.

### 6.2 Cost measurement and manipulation gate

TBD — A seed 1: steps/hour, tokens/step, projected matrix cost; frozen → A reduction; pre-tune curve if retuned.
...
- **Single task with supplied primitives.** The IR hands the model `&`, `!`, `=` and person indices; B assembles rather than invents. Disclosed; the word "discovery" is not used.
