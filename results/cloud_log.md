...
- 04:52 UTC **用户决定：A1 用 util 0.35**（理由：0.40 的约 2 GiB 余量对 400 步 / 27 h 太薄，碎片涨上去会撞同一条分配器路径；`--strict` 下崩一次要人工开机，代价远大于几个百分点的步时）。acc3（util 0.40 的全冷验收）在第 1 次启动的生成阶段被按 PID 人工停止（`acc3_cold1.rc` = stopped_by_operator；没有结果，不计入任何统计）。配置改为 0.35 并 push；启动 **acc4**（`logs/acc4.sh`）：与 acc3 相同的验收——3 次独立启动、每次启动前全部编译缓存移开（全冷）、第 1 次带小 eval（n=32）、各 1 步、util 取自配置 = 0.35。用户预先授权：3 次全过 → 直接启动 A1（旧目录另存、干净目录、以重跑自己的 step-0 为锚点），不再询问。0.40 的实测数字（acc1 / acc2 / cold2）保留在上文与 D20。
- 05:32 UTC **acc4 通过：util 0.35、全冷启动 3/3 过**（rc=0 ×3；每次编译 43–45 s = 全冷，缓存目录已是持久盘 `/workspace/.cache/vllm/…`；第 1 次带小 eval：n=32，acc 0.47，之后训练步正常）。实测（util 0.35）：KV **141,248 token**、最大并发 **13.14**；每步 494 / 508 / 508 s（gen 223–237 + train 271）；峰值 alloc 65.96 GiB、**reserved 72.85 GiB**（余量约 6.4 GiB；0.40 时 77.3、0.45 时 78.4）。三档对比：0.45 → 199k / 18.5 / 494 s / 78.4 GiB；0.40 → 170k / 15.9 / 503 s / 77.3 GiB；0.35 → 141k / 13.1 / ≈503 s / 72.9 GiB——1 步样本下 0.35 与 0.40 的步时无可分辨差别。
- 05:33 UTC **重启 stage-1 链**（用户预先授权）：pod 上崩溃现场另存——`runs/A_0.5_ord_n8_h4_d5_s1__crash0919`、`samples/…__crash0919`、`results/matrix_halt__crash0919.json`、`results/CHAIN_DONE__crash0919`；`matrix_log.md` 续写。`setsid nohup bash scripts/chain_stage1.sh`：预热（1 步 dry-run，tag `_warmup`，不计）→ A1（干净目录，先 step-0 eval；操纵门以这次 step-0 为锚点；崩溃那次 step-0 = acc 0.624 / L_mean 8219 / L_median 8586.5 / 强制收尾 39.4%，两次都报）→ 门 → B_warm-SFT(A1) → B1 → warm 判定 → `pod_stop`。配置：util 0.35、其余不变。
- **预算重算（$1.6/h）**：到此已花约 $11（A1 崩溃那次约 $2 + 排查 / 验收约 5.7 GPU·h ≈ $9）。此前"$200 = A1 / B1 各跑满 400 步"的估算没有把每 50 步一次的 eval（实测 55 min / 次）算全：每个 RL run 跑满 = 400 × 503 s = 55.9 h + 9 次 eval ≈ 8.3 h = 64 h ≈ $103；两个 ≈ $205，加已花与 SFT / 诊断 ≈ **$220–225 > $200 线**（< $300 上限）。这是"两个 run 都跑满 400 步"的最坏情形；收敛停止最早可在 step 100 触发。按用户规则在首批 eval 落地时用实测重算并上报；此处先如实记录最坏情形已高于 $200 线。
- **stage-1 内部预算线 $200 → $240**（用户 2026-09-20）：$200 是按漏算 eval 的投影划的，不是协议数字；真实上界 $220–225 < $300。首批 eval 落地时照常重算上报，投影 > $240 才停；预注册硬上限 $300 不变。看板的线同步改为 $240。
- 07:59 UTC 链状态核对：预热 rc=0；**A1 重跑的 step-0 eval**（stop 500，61.6 min）：acc **0.604**、L_mean **8261.7**、L_median 8701、到顶 41.2%、viol 0、direct 0.004（冻结 0.0045）、letter_frac 0.787。与崩溃那次（0.624 / 8219 / 8586.5 / 39.4%）及准入（0.608 / M 8599.5 / 37.8%）一致。按用户定的锚点规则，操纵门的两条线 = L_mean ≤ 0.7 × 8261.7 = **5783**、acc ≥ 0.604 − 0.05 = **0.554**（只是换算，判定以 run_matrix 为准）。A1 已完成 7 个训练步、无崩溃：均值 531 s / 步（gen ≈ 270 + train ≈ 264），峰值 reserved ≤ 73.4 GiB。
- **预算重算（实测：531 s / 步、eval 61.6 min；$1.6/h）**：每个 RL run 跑满 400 步 = 59.0 h + 9 次 eval 9.2 h = 68.2 h ≈ $109；两个 ≈ $218；加已花约 $13（崩溃那次 + 排查 / 验收 + 预热）与 SFT / 诊断估 $3–6 → 最坏情形 **≈ $235–238**，低于 $240 线（余量很小）、低于 $300。不停。步时比验收时的单步样本（≈503 s）慢约 5%：util 0.35 下并发 13 条，长 rollout 多的步生成更慢。
- **监控**：本地 watcher（`setup/auto_pull.sh`）与看板进程在跑；另挂了一个后台等待任务直接轮询 pod（每 3 min），在下列任一事件时唤醒 planning-led：A1 出现新的 eval 行、`matrix_halt.json` 出现、`CHAIN_DONE` 出现、run_matrix 进程消失、steps.jsonl 超过 40 min 未更新（eval 步为 110 min）。注意：这个等待任务依附于当前会话，会话结束即失效；看板不依赖它。
- 08:15 UTC **无人值守安排（用户睡前指示：需其过目的决策出现后倒计时 1.5 h，未回复即停机；不空转超过 1.5 h）**。新增 `scripts/pod_watchdog.sh`（D21）并在 pod 上以 setsid 常驻（`logs/watchdog.log`，每 300 s 检查）：(a) GPU 连续空转 ≥ 90 min → `pod_stop.sh --force`；(b) `/workspace/logs/DECISION_PENDING` 存在且 mtime ≥ 90 min → 停机（该文件由 planning-led 在出现需用户拍板的决策时写入，内容 = 决策摘要；用户回复后删除即取消）。两条路径都已自检：GPU 忙时 IDLE_MIN=1 跑 30 s 不误触发；用 3 分钟前的 marker + DEADLINE_MIN=1 正确触发并调用 pod_stop（自检用 stub，未真停机；marker 已删）。`pod_stop.sh --dry-run` 核对通过：pod 2dy8hd8l24yo24 RUNNING、runpodctl 与 API key 可用（真停机时用 --force，因为有 GPU 进程）。
- 分层兜底：① 链自身（`chain_stage1.sh`）在 run_matrix 结束或 `--strict` 停机后调用 `pod_stop --delay 300`——门失败 / kill / 残留触发 / 崩溃都走这条，pod 5 min 内自动停，不空转；② watchdog 的空转兜底（90 min）处理 ① 失效的情形；③ 决策倒计时 90 min。checkpoint 每 25 步，`run_matrix` 下次自动 `--resume`，停机最多丢 25 步。
- 20:10 UTC 本机异常重启（用户报告；WSL uptime 2 min）。**pod 不受影响**：链在跑（run_matrix 存活、A1 step 86、step 文件 4 min 前更新）、pod_watchdog 存活、无 halt / CHAIN_DONE、无 DECISION_PENDING。本机的 watcher 实际在 **12:12 UTC 就停了**（比重启早约 8 h，疑似机器先卡死后重启），期间看板数据停在 step 37；已用新增的 `ops/start_local.sh` 一并重启 watcher 与看板，20:09 拉到最新（step 86）。教训：本机监控是可视化，不是安全网；停机安全网在 pod 上（watchdog + 链自身的 pod_stop），本次验证了这个分工是对的。
- **A1 step-50 eval**（stop 500，58.6 min）：acc **0.640**（step-0 0.604）、L_mean **7949.0**（−3.8%）、L_median 7955、到顶 35.6%、viol 0、direct 0.004（残留阈值 0.322，远未触发）、letter_frac 0.786。训练采样趋势（每 20 步均值）：L_mean 8469 → 8237 → 8371 → 8090 → **7226**（步 81–86），reward_acc 0.548 → 0.661 → 0.644 → 0.672 → **0.781**：长度在最近 20 步才明显下行。显存：alloc 恒定 66.53 GiB，reserved 常态 73.3，**step 67 一次性冲到 78.29**（离 79.25 上限约 1 GiB，当步未崩，随后回到 73.3）；alloc 无上升趋势 → 不是泄漏，是一次性预留尖峰。util 0.35 的余量吸收掉了它（0.45 时同样的尖峰就是崩溃点）。
- **预算重算（实测 510 s/步、eval 约 60 min、$1.6/h；墙钟计费）**：已花约 $35（09-19 22:22 起连续 RUNNING 21.8 h）；A1 跑满 400 步还需 51.5 h ≈ $82；B1 全程 65.7 h ≈ $105；SFT + 诊断约 $3 → **最坏合计约 $226**，低于 $240 线（余量约 $14）、低于 $300。若 A1 提前收敛：step 100 停 → 约 $148；step 150 → 约 $161；step 200 → 约 $174。**不停，继续跑。**
- **提请注意的临近判定（不需要现在决定）**：停止判据的第一个区间 0→50 已判定为"小"（|ΔL|/L_后 = 3.93% < 5%，|Δacc| = 3.6pp < 4pp）。若 50→100 也小，A1 会在 **step 100 收敛停止**，届时 L_mean 约 7900–8000，相对 step-0 只降约 4%，**操纵门第 1 项（≥30%）必然不过 → `--strict` 停机、pod 自动停、等用户决定**（预注册的重调序列 λ=0.3 → λ=1.0 → G=32，或其他）。但步 81–86 的训练 L_mean 已降到 7226，若 step-100 eval 跟上这个下行，区间就不算小、训练继续。两种都可能，约 23:00 UTC 见分晓（step 100 ≈ 2 h + eval 1 h）。
- 20:50 UTC **D22 写定**（在 step-100 eval 之前）：门失败后按失败模式分派 (0) 停早了 → 收紧停止判据、对所有臂一律适用、从 checkpoint 继续；(1) 真压不动 → 跳过 λ=0.3 直接 λ=1.0 重跑；(2) 压过头 → λ=0.3；(3) → G=32。**待用户定的子选项**：(0) 的收紧方式二选一——变体 A「ΔL<3% 且 Δacc<3pp」或变体 B「要求三个连续小区间」。实测对照：0→50 区间 ΔL 3.93%、Δacc 3.6pp，**变体 A 下该区间即不算小**（step-100 不会停）；变体 B 保持 5%/4pp 阈值、只要求三个区间，最早停在 step 150（每 run 多约 50 步 ≈ $14）。建议 A：本次失败是"曲线还在降却被判平"，A 直接改"平"的定义，B 只是推迟——三个各降约 4% 的区间累计仍降 12%，仍会误判。
- **CoT 抽样（只读归档 `rollouts.jsonl.zst`，训练采样 T=1.0，每步 32 条，只统计正确轨迹；n 小，描述量）**：
  | step | 正确 | L 中位 | 字符中位 | 字母占比 | wait | but/however | check/verify | so/thus | 句子 | 箭头+< | 完整顺序 |
  |---|---|---|---|---|---|---|---|---|---|---|---|
  | 1 | 13/32 | 8142 | 22973 | 79.3% | 34 | 115 | 20 | 186 | 584 | 352 | 21 |
  | 25 | 8/32 | 9597 | 26357 | 78.3% | 29.5 | 132 | 23 | 215.5 | 658.5 | 419 | 23 |
  | 50 | 21/32 | 6573 | 18325 | 79.6% | 21 | 97 | 13 | 144 | 419 | 333 | 17 |
  | 75 | 26/32 | 6877 | 19817 | 79.0% | 19.5 | 101.5 | 13 | 148 | 450.5 | 340.5 | 16 |
  | 86 | 24/32 | 6495 | 18122 | 79.1% | 23.5 | 95.5 | 13.5 | 149.5 | 415 | 349 | 19.5 |
  每千字符频次（step 1 → 86）：wait 1.37 → 1.18、check/verify 0.84 → 0.71、句子 24.0 → 22.6、so/thus 8.35 → 8.30、but/however 5.11 → 4.94、if/suppose/case 3.13 → 3.19、**箭头+< 16.28 → 18.13（升 11%）**。
  **读法（描述，不进任何判定）**：到 step 86 为止，**字母占比纹丝不动（79.3% → 79.1%）**；轨迹短了约 21%，但**每千字符的成分几乎不变**——叙述标记只轻微稀释（wait −14%、check/verify −15%、句子 −6%），符号（箭头 / `<`）密度反而升 11%。即**它是"同样的写法少写一些"，不是"换一种写法"**。人工读两条中位长度的正确轨迹（step 1 vs 86）印证：开头仍是同一套 "Okay, let's try to solve this puzzle…" 样板 + 把 hard / disj 约束逐条抄一遍，结尾仍是逐条复核 5 个析取。**样板开头和结尾复核都还在，没有被压掉。** 这与 Little (2026)"长度优化先削叙述"的图景方向一致但强度弱得多——在 λ=0.5、−21% 的压缩量下，削减基本是等比例的。注意：正确率同期从 13/32 升到 24/32，长度下降与正确率上升同时发生。
  方法学注记：脚本另算了"首个完整顺序的位置 / 其后文本占比"，但该正则会命中开头抄写的约束串（step 86 中位落在 2.7% 处），**不可用作"复核段落占比"的度量**，未采信。
- 20:55 UTC **用户定：D22 (0) 的收紧方式采用变体 A（ΔL < 3% 且 Δacc < 3pp，仍是连续两个区间），变体 B 删除**。理由（用户，与 planning-led 建议一致）：B 只推迟不改"平"的定义，三个各降约 4% 的区间累计仍降约 12%，照样在下降中触发。定这条时 A1 在 **step 92、只有 step-0 / step-50 两次 eval**，step-100 数据尚未产生——分派规则与其子选项全部写定于数据之前。
  实测对照（不变）：0→50 区间 ΔL 3.93%、Δacc 3.6pp，两项在变体 A 下都不算小 → 若 (0) 触发，收紧后该区间即不成立，A1 从 checkpoint 继续。
- 22:51 UTC **A1 step-100 eval（stop 500，51.0 min）：acc 0.784、L_mean 7242.5、L_median 6972.5、到顶率 23.8%、viol 0、direct 0.0055（冻结 0.0045，残留阈值 0.394，远未触发）、letter_frac 0.7897、acc@1024 强制 0.004。**
  **收敛判据未触发，训练自动继续**：区间 0→50 为"小"（ΔL 3.93%、Δacc 3.6pp），但区间 50→100 **两项都不小**（ΔL **9.75%** > 5%，Δacc **14.4pp** > 4pp）→ 连续计数清零。**D22 (0) 未被用到**（收紧停止判据的分支这次没走）。
  三点趋势：acc 0.604 → 0.640 → **0.784**；L_mean 8261.7 → 7949.0 → **7242.5**（相对 step-0 累计 **−12.3%**）；到顶率 41.2% → 35.6% → **23.8%**；**letter_frac 0.787 → 0.786 → 0.790（三点纹丝不动）**。长度下降与准确率上升同时发生，没有精度换长度。强制到 1024 的准确率仍 ≈0 → 轨迹仍是必需的，没有内化。
  门的四项现状（门只在收敛时判，现在只是记录）：减少 **12.3%**（需 ≥30%，唯一未达标项）；acc **+18.0pp**（需损失 ≤5pp，达标且方向相反）；L_median 6972.5（需 ≥2×Kahn=60，达标）；kill 线 48.75（远未触及）。
  预算重算（510 s/步、eval 约 0.9 h、$1.6/h）：已花 24.5 h ≈ $39；A1 剩余到 400 步 47.9 h ≈ $77；B1 全程 64.8 h ≈ $104；杂项 ≈ $3 → **最坏 ≈ $223**，低于 $240 线。继续。
  监控修正：22:43 的"卡住"是误报（step-100 的 eval 占着 GPU，而判据只看训练步文件的时间）。已给监控加 GPU 计算进程检查，只有"超时且 GPU 空闲"才算异常。
- 2026-09-21 05:2x UTC **A1 step-150 eval（stop 500，44.8 min）：acc 0.852、L_mean 6636.8、L_median 6080.5、到顶率 17.6%、direct 0.0055、letter_frac 0.7814、acc@1024 0.006。收敛判据仍未触发，训练继续**（区间 100→150：ΔL **9.13%** > 5%、Δacc **6.8pp** > 4pp，两项都不小）。
  四点趋势：acc 0.604 → 0.640 → 0.784 → **0.852**；L_mean 8261.7 → 7949.0 → 7242.5 → **6636.8**（累计 **−19.7%**）；L_median 8701 → 7955 → 6973 → **6081**；到顶率 41.2% → 35.6% → 23.8% → **17.6%**；**letter_frac 0.787 → 0.786 → 0.790 → 0.781（四点仍在 78–79% 区间，无趋势）**；强制到 1024 的准确率仍 ≈0（0.004/0/0.004/0.006）→ 轨迹仍必需，未内化；direct 0.0055 对残留阈值 0.43，远未触发。
  门（只在收敛时判）：需 L_mean ≤ **5783**，现 6636.8，**累计减少 19.7%**（需 ≥30%，仍是唯一未达标项）；acc **+24.8pp**（要求是损失 ≤5pp，方向相反）；L_median 6081 ≫ 2×Kahn=60；kill 线 48.75 远未触及。近两个区间平均 7.9 pp/50 步，**外推**（非预测）约在 step 215 前后可达 30%。注意这与收敛判据存在竞争：若曲线在达到 30% 之前先变平（连续两个区间 ΔL<5% 且 Δacc<4pp），则门以"压不动"失败 → 按 D22 判 (0) 还是 (1)。
  eval 耗时随轨迹变短而下降：61.6 → 58.6 → 51.0 → 44.8 min。预算重算：已花 31.8 h ≈ $51；A1 剩余到 400 步 ≈ $63；B1 全程 ≈ $102；杂项 ≈ $3 → **最坏 ≈ $219**，低于 $240 线。继续。
- 2026-09-21 12:5x UTC **A1 step-200 eval（stop 500，40.3 min）：acc 0.844、L_mean 6162.0、L_median 5445.5、到顶率 15.0%、direct 0.007、letter_frac 0.7843、acc@1024 0.004。收敛判据仍未触发，训练继续**（区间 150→200：ΔL **7.71%** > 5% → 不小；但 Δacc **0.8pp** < 4pp → 这一项已变"小"）。
  **状态变化，值得单独记：准确率到顶了**（0.784 → 0.852 → **0.844**，最后一步 −0.8pp，n=500 的 SE ≈ 1.6pp，属噪声）。此后 Δacc 大概率每个区间都算"小"，**收敛与否从此只取决于长度轴**。各区间 ΔL：9.75% → 9.13% → **7.71%**，在递减但仍高于 5%。因需连续两个小区间，最早停在 **step 300**（需 200→250 与 250→300 都小）。
  五点趋势：acc 0.604/0.640/0.784/0.852/0.844；L_mean 8261.7/7949.0/7242.5/6636.8/**6162.0**（累计 **−25.4%**）；L_median 8701/7955/6973/6081/**5446**；到顶率 41.2%/35.6%/23.8%/17.6%/**15.0%**；**letter_frac 0.787/0.786/0.790/0.781/0.784（五点无趋势）**；强制@1024 acc 仍 ≈0 → 未内化；direct 0.007 对残留阈值 0.424，远未触发。
  **门已进入射程**：需 L_mean ≤ 5783，现 6162.0，只差 **6.2%** 的相对下降；最近一个区间的降幅是 7.7%。若 200→250 保持同等降幅，step-250 即可越过 30% 线——但门只在**收敛停止时**判，不是一越线就判。
  eval 耗时 61.6/58.6/51.0/44.8/**40.3** min。预算：已花 38.8 h ≈ $62；A1 剩余到 400 步 ≈ $49；B1 全程 ≈ $99；杂项 ≈ $3 → **最坏 ≈ $213**，低于 $240 线。继续。
- 2026-09-21 19:4x UTC **A1 step-250 eval（stop 500，35.0 min）：acc 0.924、L_mean 5651.3、L_median 5007.5、到顶率 9.0%、direct 0.008、letter_frac 0.7897、acc@1024 0.006。收敛判据仍未触发**（区间 200→250：ΔL **9.04%** > 5%、Δacc **8.0pp** > 4pp，两项都不小）→ 最早停在 **step 350**。
  **门的长度条件已达标**：需 L_mean ≤ 5783，现 **5651.3 → 累计减少 31.6% ≥ 30%**。门的另三项同时满足：acc **+32.0pp**（要求是损失 ≤5pp）、L_median 5007.5 ≫ 2×Kahn=60、kill 线 48.75 未触及。**但门只在收敛停止时按最后一次 eval 判**，所以真正决定的是停下来那一刻的 L_mean；目前趋势向下，继续跑对门有利。
  **更正我在 step-200 的读法**：当时写"准确率到顶了"（0.852 → 0.844），step-250 是 **0.924**。0.844 是单点下行（n=500，SE≈1.6pp），不是平台。据此我当时推的"此后 Δacc 每区间都算小、收敛只取决于长度轴"也不成立——200→250 的 Δacc 是 8.0pp。
  六点趋势：acc 0.604/0.640/0.784/0.852/0.844/**0.924**；L_mean 8261.7/…/**5651.3**（**−31.6%**）；L_median 8701/…/**5008**；到顶率 41.2%/35.6%/23.8%/17.6%/15.0%/**9.0%**；**letter_frac 0.787/0.786/0.790/0.781/0.784/0.790（六点仍无趋势）**；强制@1024 仍 ≈0；direct 0.008 对残留阈值 0.466，远未触发。
  分解表更新到 step 250（`ops/notes_accuracy_decomposition.md`）：0→250 合计 +32.0pp = 到顶率下降 **+24.4pp（76%）** + 组内变化 +7.6pp。自然结束者 acc 0.922 → **0.991**。被截断组 acc 在 0.080–0.295 间跳（n 从 206 降到 45），该列仍不可跨步比较。
  eval 耗时 61.6/58.6/51.0/44.8/40.3/**35.0** min。预算：已花 45.3 h ≈ $72；A1 剩余到 400 步 ≈ $36；B1 全程 ≈ $96；杂项 ≈ $3 → **最坏 ≈ $207**，低于 $240 线。继续。
- 2026-09-22 02:1x UTC **A1 step-300 eval（stop 500，33.4 min）：acc 0.924（与 step-250 完全相同）、L_mean 5453.4、L_median 4828.0、到顶率 9.0%（持平）、direct 0.008、letter_frac 0.7871。**
  **首次出现"小区间"**：250→300 的 ΔL **3.63%** < 5%、Δacc **0.0pp** < 4pp → 两项都小，**连续计数 = 1**。各区间 ΔL：3.93 / 9.75 / 9.13 / 7.71 / 9.04 / **3.63**%。**若 300→350 也小，A1 在 step 350 判定收敛并停止。**
  **届时门按 step-350 的 eval 判，目前四项都满足且余量充足**：需 L_mean ≤ 5783，现 **5453.4 → 减少 34.0%**（余量 330 token，即 L 反弹 6% 以内都仍达标）；acc 0.924（**+32.0pp**，要求是损失 ≤5pp）；L_median 4828 ≫ 2×Kahn=60；kill 线 48.75 未触及。
  **若门通过，链自动继续，无需用户决定**：A1 → 06_diagnose → B_warm-SFT(A1)（E2 早读）→ B1 → ≤200 步 warm 判定 → 停机。**D22 的重调分派届时不会被用到。**
  分解表更新到 step 300：0→300 合计 +32.0pp = 到顶率下降 **+25.2pp（79%）** + 组内 +6.8pp；自然结束者 0.922 → **0.996**；250→300 区间总变化 0.0pp（到顶率与 acc 都持平）。
  六→七点趋势：letter_frac 0.787/0.786/0.790/0.781/0.784/0.790/**0.787**——**七个点全部落在 78.1–79.0%，压缩 34% 全程未改变字符构成**。eval 耗时 61.6→**33.4** min。预算：已花 51.9 h ≈ $83；A1 到 350 步 + 诊断 ≈ $13；B_warm-SFT ≈ $1；B1 全程 + 诊断 ≈ $95 → **最坏 ≈ $192**，低于 $240 线。继续。
- 2026-09-22 **文档与描述量（用户指示；不改任何判定，链照常跑）**：① §2 的 Kaufmann et al. 行按用户读完全文后的更正重写（DeepMind 的 CoT 可监控性框架论文而非效率论文；hinge 惩罚 −max(0,|c|−L_min)；Qwen3-8B/14B 与 Gemma-3、no_think、token 级 GRPO/CAPO；H&T 按字符计→塌缩成 `H→T→H`，Backdoor-Easy 按 token 计→轨迹塌缩成空/单词而代码仍对；**"任务是否认证需要轨迹"改为"否"**），§1 与 §9 中依赖该行的两处表述一并更正（原文把 Little 与 Kaufmann 并列说成"在推理必需的任务上"）。② §7.4 加三条（长度压力正比于准确率与双向偏差、A 压缩但未改写法、token 计价 vs 字符计价）。③ §9 待读清单加 Brown-Cohen et al. 2026 (2603.09786, opaque serial depth) 与 Emmons et al. 2025a，标注与 §4.2 准入认证直接相关、核对后再决定是否进 §2。
  **新描述量（`ops/notes_length_distribution.md`，脚本 `ops/analysis/len_quantiles.py`、`length_signal.py`）**：
  - 长度分位数 0→300。**仅自然结束者**：p10 −32.2%、p25 −32.5%、p50 −32.1%、p75 −28.8%、p90 −19.3% —— **主体整体左移且各分位降幅一致 → "压缩全部来自解除 cap 截断"的强版本不成立**。但全部 500 条的 **p90 只降 4.8%**（10240→9752），约一成轨迹仍顶在 cap 附近，cap 对长尾的压力未消失。成分论证：自然结束子群从 294 条长到 455 条，新进来的正是原先最长的题，会把 step-300 分布往上拉 → 真实同题内压缩只会更大。
  - 长度信号占组内总奖励差的比例（每组 16 条）：0.390 → 0.426 → 0.429 → 0.472 → 0.496 → **0.500**（1–50 → 251–300 步）；组内答对数中位 11 → 15；全对组 8% → 42%；全错组恒为 0。→ **A 在 92% 正确率下约一半梯度信号来自长度**；准确率低的臂按比例拿到更少的长度优化，这是 §7.4 新增第一条的量化依据。B 跑起来后用同一脚本算，两臂并排。

## 2026-09-22 08:4x UTC A1 收敛（step 350）
- **A1 step-350 eval（stop 500，31.6 min）：acc 0.912、L_mean 5289.6、L_median 4539.5、到顶率 9.4%、direct 0.0065、letter_frac 0.7786。**
  区间 300→350：ΔL **3.10%** < 5%、Δacc **1.2pp** < 4pp → 小；与 250→300 构成**连续两个小区间 → 收敛停止**。`designated_ckpt.json` = `{"step": 350, "reason": "converged", "max_steps": 400}`。A1 未跑满 400 步。
- **操纵门（按最后一次 eval = step-350 判；run_matrix 在诊断之后写入 matrix_log，此处为独立复算）**：
  | 门的项 | 值 | 阈值 | 判 |
  |---|---|---|---|
  | token 减少（相对 step-0 L_mean 8261.7） | **35.97%** | ≥30% | 过 |
  | acc 损失 | **−30.8pp**（即上升 30.8pp） | ≤5pp | 过 |
  | 收敛 L_median | **4539.5** | ≥2×Kahn = 60 | 过 |
  | kill（L_median ≤ 1.5×决策下限 48.75） | 4539.5 | — | 未触发 |
  → **预期门通过，链自动进 B_warm-SFT(A1)【E2】→ B1。D22 的重调分派未被用到；λ=0.5、G=16 维持不变并适用于所有臂。**
- A1 全程（八个 eval 点）：acc 0.604 → 0.912；L_mean 8261.7 → **5289.6（−35.97%）**；L_median 8701 → **4539.5（−47.8%）**；到顶率 41.2% → 9.4%；**letter_frac 0.787/0.786/0.790/0.781/0.784/0.790/0.787/0.779 —— 八个点全部落在 77.9–79.0%，压缩 36% 全程未改变字符构成**；强制@1024 acc 始终 ≈0（轨迹未内化）；direct 0.004 → 0.0065（残留阈值 0.459，从未接近）。
- 准确率分解更新到 350：0→350 合计 +30.8pp = 到顶率下降 **+26.0pp（84%）** + 组内 +4.8pp。300→350 这一格总变化 −1.2pp（acc 0.924 → 0.912，n=500 的 SE≈1.3pp，属噪声）。
...
- 2026-09-22 13:0x UTC **B1 前 10 步（320 条 rollout）：开局被"逃逸屏蔽"主导，不是无梯度。更正我上一条的预判。**
  - 违规构成：**hard 违规 33.4%**（`text_after_think` 107 条、`second_think_tag` 68 条、`bad_answer` 3 条；无违规 210 条）。`reward_hard` 每步 0.31–0.34。
  - **`</think>` 的出现位置：p10 = 第 5 个字符**、中位 1172、p90 13821 → 至少一成 rollout **几乎立刻写 `</think>` 逃出屏蔽**，然后在解除屏蔽的答案区用完整英文推理。抓到的样本开头：`**<|100000|>**\n\n**<|100000|>**\n</think>\n\nTo solve the puzzle, we follow the constraints…`。这正是 2026-09-17 记录过、并为此把该违规定为 hard（r = −5）的行为。
  - **正确率 c=1 仅 0.003（320 条里 1 条）**；think 段 L 中位 1236（step-0 eval 是 9926），到顶率 0.369。
  - **组内退化：零奖励差（无梯度）组 0/20 = 0.0%，组内奖励差中位 5.00；全错组 95.0%（臂 A 全程 0%）。**
    **更正**：我上一条预判"acc 1.4% → 绝大多数组 r 全为 0 → 无梯度"，实测不成立——奖励差来自 **−5（逃逸）与 0（干净但答错）之间**，所以梯度存在且很强。但**这个梯度教的是"别逃逸"，不是"做对"也不是"写短"**。长度信号占比在 B 上目前接近 0（答对者几乎不存在）。
  - 读法（描述，不进判定）：B1 开局的优化目标实际上是格式合规。它能否在此之后学会任务，是 ≤200 步 warm 判定要回答的问题（阈值：最佳 acc < 0.924 − 15pp = 0.774 → 冷启动失败 → B_warm-RL 待跑）。
  - 附带观察（进 §6.6 素材）：逃逸样本里出现 `<|100000|>` 这类**解码后只含数字与标点、因而合法在白名单内**的保留 token。不违反任何规则，但属于"B 写了什么"的值得记录的现象。
- 2026-09-22 14:4x UTC **用户裁决 E2：选 (a)（D25）**——test 8 对齐注册原文（100 条、各类占比差 ≤1pp），warm 映射与检测器不动。pod 上 dry-run 验证：`test8_n=100, test8_max_gap_pp=1.0, tolerance 1.0` → rc=0，**压线通过**（seed=0 的 100 条恰含那 1 条翻转轨迹）。**E2 排在 B1 之后自动跑**：`/workspace/logs/chain_e2_after_b1.sh` 等 `chain_b1` 写出 CHAIN_DONE → 取消其 pod_stop 倒计时 → `run_matrix --matrix configs/matrix_stage1_e2.yaml --strict` → 停机。GPU 没有空档（B1 占 62–77 GB），故不并行。
- **看板第 2 块加 B1 面板**（用户要求）：pod 上 `ops/analysis/b_agg.py` 每 10 min 从 B1 的 rollouts 归档按每 10 步聚合 hit_exact / hit_hamming2（与 eval.jsonl 同口径）/ hard 违规率 / 零梯度组占比 / 全错组占比 → `runs/B_…/agg10.json`；watcher 的曲线拉取加入该文件（D19 的延伸）；看板画三张图。首批数据（训练 rollouts，T=1.0）：
  | 步 | hit_exact | Hamming≤2 | **hard 违规** | **零梯度组** | 全错组 |
  |---|---|---|---|---|---|
  | 0 | 0.000 | 0.031 | 0.250 | 0.000 | 1.00 |
  | 1–10 | 0.009 | 0.034 | 0.328 | 0.000 | 0.90 |
  | 11–20 | 0.009 | 0.019 | **0.100** | **0.100** | 0.90 |
  | 21–30（部分） | 0.000 | 0.021 | **0.031** | **0.333** | 1.00 |
  **读法**：B 正在学会不逃逸（hard 违规 33% → 3%），而**唯一的奖励差正是来自逃逸的 −5**；逃逸一消失，组内 r 全为 0，无梯度组随之从 0% 升到 33%。命中率纹丝不动（≈0）。这是 §7.4 第一条与 `ops/notes_group_degeneracy.md` 预判的机制在 B 上的实时呈现：**格式合规学会之后，B 面临的是零信号，而不是弱信号**。是否为冷启动失败由 ≤200 步 warm 判定决定（阈值 0.774），不在此处下结论。
