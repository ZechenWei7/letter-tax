...
def test_analyze_e1_e2_e3(synth_runs):
    ...
    E1 = R["E1"]
    assert E1["matched"]["computable"] and E1["matched"]["y_star"] == pytest.approx(min(0.80, 0.77, 0.76) - 0.05)          # v8：y = 总准确率
    tk = E1["tax"]["tokens"]
    assert set(tk["per_seed"]) == {1, 2, 3} and tk["tier"] in ("moderate", "strong") and 0.4 < tk["point"] < 0.8 and tk["lo"] < tk["point"] < tk["hi"]
    ...
    assert R["direct_comparable_le_3pp"]["ok"]
...
def test_analyze_uncomputable_and_flags(synth_runs):
    ...
    for v in runs["A"].values():
        v["diag"]["budget_points"] = {"1.0": dict(acc=0.99, externalized=0.94, mean_tokens_correct=1500, mean_tokens_all=1500, n_correct=50)}   # A 只有远高于 y* 的点
    R = az.analyze(runs, key, frozen_direct=0.02)
    assert not R["E1"]["matched"]["computable"] and "tax" not in R["E1"] and R["E1"]["verdict"].startswith("E1 不可计算") and R["E1"]["no_match_table"]["A"]["1"]["top_y"] == 0.99
    assert R["uninterpretable_arms"] == ["B"] and not R["direct_comparable_le_3pp"]["ok"]
