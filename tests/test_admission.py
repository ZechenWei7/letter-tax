...
def base():
    return dict(chance=1 / 1680, d=5, direct=0.03, direct_n=2000, direct_lenient=0.035, native=0.70, native_lenient=0.72, M=3000,
                ...
                transplant_acc=0.10, template_hit_rate=0.30, audit_template_rate=0.01, masked_frozen_acc=0.03,
...
def test_each_criterion_fails():
    for k, v in [("direct", 0.07), ("native", 0.85), ("transplant_acc", 0.40), ("audit_template_rate", 0.05), ("masked_frozen_acc", 0.30), ("M", 100.0)]:
        m = base(); m[k] = v
...
def test_pending_when_shortcut_or_kahn_or_d_missing():
    m = base(); m["audit_template_rate"] = None; m["kahn_lb"] = None; m["d"] = None
    d = admission_decision(m)
    assert not d["admitted"] and set(d["pending"]) == {"1_direct_le_2x_2^-d", "7_template_audit_lt_5pct", "8_M_ge_5x_kahn_c2.5"}
