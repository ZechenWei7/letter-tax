...
metrics 由 01_calibrate.py --admission 在 stopping-eval 500 题（split="stop"）上测得：
  direct（严格；**direct-check 2000 题**上测）, direct_lenient, d（析取条数）, native（严格，S≥1 总体）, native_lenient, chance = 1/#LE(hard)（格中位，只报）, M（500 条原生轨迹中位长度；训练的 M 即此值）,
...
E1 要求臂间训练后直接作答差 ≤3pp，否则标 "not comparable"（analyze.py）。
"""
...
ORACLE_MULT = 5.0
ORACLE_C = "2.5"
DIRECT_MULT = 2.0          # 第 1 条：direct ≤ 2 × 2^−d

def derangement(n: int, rng) -> list[int]:
    """均匀随机错位：π(i) ≠ i 对所有 i（拒绝采样）。移植时题 i 用题 π(i) 的原生轨迹。"""
    assert n >= 2
    while True:
...
def admission_decision(m: dict, native_lo: float = 0.60, native_hi: float = 0.80) -> dict:
    ...
    d_ = m.get("d"); thr1 = (DIRECT_MULT * 2.0 ** (-int(d_))) if d_ is not None else None
    checks["1_direct_le_2x_2^-d"] = dict(ok=(None if thr1 is None else direct <= thr1), direct=direct, direct_lenient=m.get("direct_lenient"), d=d_, threshold=thr1,
                                         direct_n=m.get("direct_n"), chance_1_over_LE_reported_only=chance,
