...
def detect_shortcuts(think: str, prompt: str, n: int, d: int) -> list[str]:
    hits = []
    ...
    for m in re.finditer(r"\d+(?:[ \t]*(?:[ ,<→>\-]|->)[ \t]*\d+)+", think):
        vals = [int(x) for x in re.findall(r"\d+", m.group(0))]
        if len(vals) == n and sorted(vals) == list(range(n)) and _VERIFY.search(think[m.end():m.end() + 80]): gv += 1
    if gv >= 3: hits.append("guess_verify")
    return hits
...
def shortcut_rate(thinks, prompts, n, d) -> dict:
    hs = [detect_shortcuts(t, p, n, d) for t, p in zip(thinks, prompts)]
